...
# regular rendering as seen in tutorial with hdri and kps - no depth
...
parser.add_argument('--output_dir', default="tmp_hdri", help="Output root") 
...
def project_world(points_xyz, cam2world, K): 
    ...
    Pw = np.concatenate([points_xyz, np.ones((len(points_xyz),1))], axis=1)
    # (N,4) 
...
for cls_name, blend_path in blend_jobs:
    ...
    kp_world_static = np.array(kp_world_static) 
    # (K,3) 
    M_tool = tool.get_local2world_mat() 
    bbox_local = np.array(tool.get_bound_box()) 
    # 8x3 
